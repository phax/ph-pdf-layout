...
/**
 * Render-level tests for {@link PLRichText}. These exercise the full prepare / render pipeline by
 * writing a real PDF to a temp file and asserting the file is non-empty. Pixel-diff reference
 * comparisons are deliberately not wired up yet — that happens in a later step.
 *
...
public final class PLRichTextRenderTest
{
  ...
  public void testWrapping () throws IOException, PDFCreationException
  {
    ...
    final PLRichText aRT = PLRichText.createFromMarkup (aSB.toString ().trim (), FONT_FAMILY, 12f, PLColor.BLACK);
