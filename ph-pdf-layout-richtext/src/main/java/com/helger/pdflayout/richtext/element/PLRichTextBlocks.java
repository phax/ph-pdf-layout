...
/**
 * Block-level companion to {@link PLRichText}. Parses a markup string that may
 * contain line-prefix indent markers and turns it into a list of
 * {@link IPLElement}s, one per paragraph or list item. The intended use is to
 * add the returned elements directly to a
 * {@link com.helger.pdflayout.base.PLPageSet}.
 *
 * <p>Recognised line-prefix markers (verbatim from the original
 * {@code rst.pdfbox.layout} library):</p>
 * <ul>
 * <li>{@code --} — plain indent block (no label)</li>
 * <li>{@code -+} — bullet list item (default bullet {@code •})</li>
 * <li>{@code -#} — numbered list item (default arabic, restarts after a reset
 * or whenever the markup switches away from {@code -#})</li>
 * <li>{@code -!} — reset / end indentation block; produces no element</li>
 * <li>Leading spaces before the marker increase the indent level by one each.
 * Maximum supported depth is 3 levels.</li>
 * </ul>
 *
 * <p>Each block's inline content is itself markup and is rendered via
 * {@link PLRichText#createFromMarkup}, so bold/italic/colour/sub-superscript
 * markup remain available inside the block.</p>
 *
 * <p>This is intentionally a "structurally equivalent" port of the original
 * indentation feature — bullets render as bullets, numbers as numbers,
 * indentation is applied via {@code marginLeft} on the produced element — but
 * the pixel-perfect appearance of the original is not reproduced (no
 * right-aligned label cells, no per-level bullet glyph swap).</p>
 *
...
public final class PLRichTextBlocks
{
  ...
  /**
   * Parse a markup string into a list of block-level elements. The result can
   * be added one-by-one to a {@link com.helger.pdflayout.base.PLPageSet}.
   *
   * @param sMarkup
  ...
  private static IndentMarker _parseIndentMarker (@NonNull final String sLine)
  {
    ...
    final EIndentKind eKind;
    switch (sMarker)
    {
      case "--":
        eKind = EIndentKind.PLAIN;
        break;
      case "-+":
        eKind = EIndentKind.BULLET;
        break;
      case "-#":
        eKind = EIndentKind.NUMBERED;
        break;
      case "-!":
      default:
        eKind = EIndentKind.RESET;
        break;
    }
